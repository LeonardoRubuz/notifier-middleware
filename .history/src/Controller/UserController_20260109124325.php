...
final class UserController extends AbstractController
{
    ...
    public function new(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Génération du mot de passe AVANT l'envoi de l'email
            $plainPassword = $this->generatePassword();

            // Stockage temporaire des données en session (PAS EN BD)
            $session->set('pending_user_' . $token, [
                'nom' => $user->getNom(),
                'prenom' => $user->getPrenom(),
                'username' => $user->getUsername(),
                'roles' => $user->getRoles(),
                'password' => $plainPassword, // Mot de passe en clair (temporaire)
    ...
    ): Response
    {
        ...
        // Création de l'utilisateur MAINTENANT
        $user = new User();

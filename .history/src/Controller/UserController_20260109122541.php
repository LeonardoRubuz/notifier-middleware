...
final class UserController extends AbstractController
{
    ...
    public function new(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // 1️ L'utilisateur n'est PAS encore vérifié
            $user->setIsVerified(false);

            // 2Génération du token email
...
): Response
{
    ...
    $passwordMail = (new TemplatedEmail())
        ->from(new Address('payroll@flexpaie.com', 'FlexRoll'))
        ->to($user->getUsername())
        ->subject('Votre mot de passe FlexRoll')
        ->htmlTemplate('flexroll/password.html.twig')
        ->context([
            'password' => $plainPassword,
        ]);

    $mailer->send($passwordMail);

    $this->addFlash('success', 'Email confirmé. Mot de passe envoyé.');
